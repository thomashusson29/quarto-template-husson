...
# Initialisation du chemin systeme et des utilitaires Quarto / XeLaTeX
EXTRA_SYSTEM_PATHS = [
...
def ensure_complete_project_environment(target_path: Path):
    """
    Deploie de facon totalement autonome l'integralite des ressources requises
    autour du fichier .qmd sans dependre de la configuration prealable de l'utilisateur :
    1. Extension physique _extensions/husson (dossier reel, pas de lien symbolique).
    2. Logos institutionnels (Paris-Saclay, Paris Cite) copies dans le dossier et dans images/.
    3. Fichiers de polices et bibliographies partages.
    4. Fichier _quarto.yml a la racine du projet et localement pour la reconnaissance Positron.
    """
    if not TEMPLATE_EXTENSION_DIR.exists():
        return False, f"Dossier source de l'extension Husson introuvable : {TEMPLATE_EXTENSION_DIR}"
    ...
    logo_filenames = [
        "logo-universite-paris-saclay.png",
        "logo-universite-paris-cite.png"
    ]

    for loc in locations:
        # 1. Installation de l'extension physique
        ext_dir = loc / "_extensions"
        husson_dir = ext_dir / "husson"

        if not (husson_dir.resolve() == TEMPLATE_EXTENSION_DIR.resolve() and not husson_dir.is_symlink()):
        ...
        # 2. Deploiement des logos dans la racine locale et dans images/
        images_dir = loc / "images"
        try:
            images_dir.mkdir(parents=True, exist_ok=True)
            for lf in logo_filenames:
                src_logo = TEMPLATE_EXTENSION_DIR / lf
                if src_logo.exists():
                    dest_direct = loc / lf
                    if not dest_direct.exists():
                        shutil.copy2(src_logo, dest_direct)
                    dest_images = images_dir / lf
                    if not dest_images.exists():
                        shutil.copy2(src_logo, dest_images)
        except Exception:
            pass

        # 3. Deploiement des polices locales
        fonts_src = TEMPLATE_EXTENSION_DIR / "fonts"
        if fonts_src.exists():
            fonts_dst = loc / "fonts"
            try:
                fonts_dst.mkdir(parents=True, exist_ok=True)
                for font_file in fonts_src.glob("*.ttf"):
                    if not (fonts_dst / font_file.name).exists():
                        shutil.copy2(font_file, fonts_dst / font_file.name)
                    if not (loc / font_file.name).exists():
                        shutil.copy2(font_file, loc / font_file.name)
            except Exception:
                pass

        # 4. Bibliographie partagee de repli
        bib_src = TEMPLATE_EXTENSION_DIR / "biblio_shared.bib"
        if bib_src.exists():
            bib_dst = loc / "biblio_shared.bib"
    ...
    # 5. Creation automatique de _quarto.yml
    ...
    return True, "Environnement complet et extension Husson installes avec succes."


ensure_extension_installed = ensure_complete_project_environment


def check_husson_readiness(yaml_data: dict) -> tuple:
    """
    Verifie si le document est pret pour le rendu avec le template Husson.
    Retourne (est_pret: bool, liste_des_raisons: list[str]).
    """
    if not isinstance(yaml_data, dict):
        return False, ["Structure YAML invalide."]

    reasons = []
    formats = yaml_data.get("format", {})
    if isinstance(formats, str):
        formats = {formats: {}}
    elif not isinstance(formats, dict):
        formats = {}

    if "husson-pdf" not in formats:
        reasons.append("Format 'husson-pdf' manquant (actuel : " + (", ".join(formats.keys()) if formats else "aucun") + ").")
    else:
        first_format = next(iter(formats.keys()), None)
        if first_format != "husson-pdf":
            reasons.append("Format 'husson-pdf' doit figurer en premiere position pour Positron.")

        pdf_conf = formats.get("husson-pdf", {})
        if isinstance(pdf_conf, dict):
            if pdf_conf.get("documentclass") != "scrartcl":
                reasons.append("Classe de document 'scrartcl' manquante sous husson-pdf.")
            if pdf_conf.get("pdf-engine") != "xelatex":
                reasons.append("Moteur 'xelatex' manquant sous husson-pdf.")

    thesis_block = yaml_data.get("thesis")
    if isinstance(thesis_block, dict):
        logo_path = str(thesis_block.get("logo", ""))
        if "_extensions" in logo_path or logo_path.startswith("_"):
            reasons.append("Chemin de logo avec underscore '_extensions' a normaliser.")

    return (len(reasons) == 0), reasons


def auto_configure_husson_document(target_path: Path, yaml_data: dict, markdown_body: str):
    """
    Transforme automatiquement et sans perte le YAML d'un document quelconque
    pour lui appliquer l'integralite des standards du template Husson.
    Sauvegarde preventiment l'original dans scratch/.
    """
    d = dict(yaml_data) if isinstance(yaml_data, dict) else {}

    # Metadonnees de base
    if not d.get("author"):
        d["author"] = "Thomas Husson"
    if not d.get("lang"):
        d["lang"] = "fr"
    ...
    if d.get("lang") == "fr":
        if "language" not in d or not isinstance(d["language"], dict):
            d["language"] = {}
        if "fr" not in d["language"] or not isinstance(d["language"]["fr"], dict):
            d["language"]["fr"] = {}
        d["language"]["fr"]["crossref-tbl-title"] = "Tableau"
        d["language"]["fr"]["crossref-tbl-prefix"] = "Tableau"
        d["language"]["fr"]["crossref-lot-title"] = "Liste des tableaux"

    # Extraction ou resolution de la police
    existing_font = d.get("mainfont", d.get("font", None))
    raw_formats = d.get("format", {})
    if isinstance(raw_formats, str):
        raw_formats = {raw_formats: {}}
    elif not isinstance(raw_formats, dict):
        raw_formats = {}

    for fk in ("husson-pdf", "pdf"):
        if fk in raw_formats and isinstance(raw_formats[fk], dict):
            if not existing_font:
                existing_font = raw_formats[fk].get("mainfont", None)

    chosen_font = existing_font if existing_font else "Times New Roman"
    d["mainfont"] = chosen_font

    # Preparation du format husson-pdf
    pdf_base = {}
    if "husson-pdf" in raw_formats and isinstance(raw_formats["husson-pdf"], dict):
        pdf_base = dict(raw_formats["husson-pdf"])
    elif "pdf" in raw_formats and isinstance(raw_formats["pdf"], dict):
        pdf_base = dict(raw_formats["pdf"])

    pdf_base["pdf-engine"] = "xelatex"
    pdf_base["documentclass"] = "scrartcl"
    pdf_base["mainfont"] = chosen_font
    if "toc" not in pdf_base:
        pdf_base["toc"] = True
    if "toc-depth" not in pdf_base:
        pdf_base["toc-depth"] = 3
    if "number-sections" not in pdf_base:
        pdf_base["number-sections"] = True
    if "fontsize" not in pdf_base:
        pdf_base["fontsize"] = "11pt"
    if "linestretch" not in pdf_base:
        pdf_base["linestretch"] = 1.15

    # Nettoyage des inclusions et filtres manuels redondants
    partials = [p for p in pdf_base.get("template-partials", []) if "husson" not in str(p) and "before-body.tex" not in str(p)]
    if partials:
        pdf_base["template-partials"] = partials
    elif "template-partials" in pdf_base:
        del pdf_base["template-partials"]

    headers = [h for h in pdf_base.get("include-in-header", []) if "pdf-header.tex" not in str(h)]
    if headers:
        pdf_base["include-in-header"] = headers
    elif "include-in-header" in pdf_base:
        del pdf_base["include-in-header"]

    flts = [f for f in pdf_base.get("filters", []) if "husson" not in str(f) and not any(k in str(f) for k in ("typography", "table-column-widths", "code-language", "heading-levels"))]
    if flts:
        pdf_base["filters"] = flts
    elif "filters" in pdf_base:
        del pdf_base["filters"]

    if d.get("lang") == "fr":
        if "language" not in pdf_base or not isinstance(pdf_base["language"], dict):
            pdf_base["language"] = {}
        if "fr" not in pdf_base["language"] or not isinstance(pdf_base["language"]["fr"], dict):
            pdf_base["language"]["fr"] = {}
        pdf_base["language"]["fr"]["crossref-tbl-title"] = "Tableau"
        pdf_base["language"]["fr"]["crossref-tbl-prefix"] = "Tableau"
        pdf_base["language"]["fr"]["crossref-lot-title"] = "Liste des tableaux"

    # Preparation du format husson-html
    html_base = {}
    if "husson-html" in raw_formats and isinstance(raw_formats["husson-html"], dict):
        html_base = dict(raw_formats["husson-html"])
    elif "html" in raw_formats and isinstance(raw_formats["html"], dict):
        html_base = dict(raw_formats["html"])

    if "toc" not in html_base:
        html_base["toc"] = True
    if "toc-depth" not in html_base:
        html_base["toc-depth"] = 3
    if "number-sections" not in html_base:
        html_base["number-sections"] = True
    if "theme" not in html_base:
        html_base["theme"] = "cosmo"
    html_base["embed-resources"] = True

    # Ordonnancement : husson-pdf STRICTEMENT en premier
    new_formats = {
        "husson-pdf": pdf_base,
        "husson-html": html_base
    }
    for k, v in raw_formats.items():
        if k not in ("pdf", "html", "husson-pdf", "husson-html"):
            new_formats[k] = v
    d["format"] = new_formats

    # Normalisation du bloc thesis si present
    if "thesis" in d and isinstance(d["thesis"], dict):
        th = dict(d["thesis"])
        cur_logo = str(th.get("logo", ""))
        if not cur_logo or "_extensions" in cur_logo or cur_logo.startswith("_"):
            th["logo"] = "images/logo-universite-paris-saclay.png"
        if not th.get("university"):
            th["university"] = "Université Paris-Saclay"
        if not th.get("faculty"):
            th["faculty"] = "Faculté de Médecine"
        d["thesis"] = th

    # Sauvegarde avec copie de securite
    ok, msg, bpath = save_qmd_with_backup(target_path, d, markdown_body)
    return ok, msg, bpath, d
...
# Verification et installation complete de l'environnement physique et des dependances
env_ok, env_msg = ensure_complete_project_environment(target_path)

# Lecture initiale du document .qmd
yaml_data, markdown_body, err = parse_qmd(target_path)
if err:
    st.error(err)
    st.stop()

# Verification de la conformite avec le template Husson
is_ready, reasons = check_husson_readiness(yaml_data)

# Auto-configuration autonome immediate si le document n'est pas configure Husson
auto_converted_msg = None
if not is_ready:
    conv_ok, conv_msg, conv_bpath, yaml_data = auto_configure_husson_document(target_path, yaml_data, markdown_body)
    if conv_ok:
        auto_converted_msg = f"Template Husson configure automatiquement ! (format husson-pdf active en premier, XeLaTeX, scrartcl, polices et logos locaux deployes). Copie preventive : scratch/{conv_bpath.name}"
    else:
        st.warning(f"Configuration automatique : {conv_msg}")
...
with col_info2:
    if auto_converted_msg:
        st.success("Template Husson applique", icon=None)
    elif env_ok:
        st.success("Template Husson actif", icon=None)
    else:
        st.warning(env_msg, icon=None)

if auto_converted_msg:
    st.success(auto_converted_msg, icon=None)

# Extraction des donnees YAML existantes
cur_title = yaml_data.get("title", "")
cur_subtitle = yaml_data.get("subtitle", "")
raw_author = yaml_data.get("author", "Thomas Husson")
if isinstance(raw_author, list):
    author_items = []
    for a in raw_author:
        if isinstance(a, dict):
            author_items.append(a.get("name", ""))
        else:
            author_items.append(str(a))
    cur_author = ", ".join(filter(None, author_items))
elif isinstance(raw_author, dict):
    cur_author = raw_author.get("name", "Thomas Husson")
else:
    cur_author = str(raw_author or "Thomas Husson")
...
# Detection de la police par defaut (defaut Husson : Times New Roman)
...
with tab_cover:
    ...
    if st.session_state.opt_thesis_active:
        ...
        with col_th1:
            th_university = st.text_input("Universite :", value=str(thesis_data.get("university") or "Université Paris-Saclay"))
            th_faculty = st.text_input("Faculte :", value=str(thesis_data.get("faculty") or "Faculté de Médecine"))
            th_degree = st.text_input("Intitule du Diplome / Formation :", value=str(thesis_data.get("degree") or "Master 2 -- Santé Publique & Méthodologie de la Recherche"))
            th_laboratory = st.text_input("Laboratoire / Unite de recherche :", value=str(thesis_data.get("laboratory") or ""))
        with col_th2:
            th_specialty = st.text_input("Specialite / UE / Groupe :", value=str(thesis_data.get("specialty") or "UE Épidémiologie -- Groupe 52"))
            th_dept = st.text_input("Departement / Service / Context :", value=str(thesis_data.get("department") or "Étude observationnelle SUPPORT"))
            th_date = st.text_input("Date / Annee universitaire :", value=str(thesis_data.get("date") or "Année universitaire 2025-2026"))

        col_logo1, col_logo2 = st.columns([2, 2])
        with col_logo1:
            LOGO_OPTIONS = ["Université Paris-Saclay", "Université Paris Cité", "Aucun logo", "Personnalise..."]
            cur_logo = str(thesis_data.get("logo", "images/logo-universite-paris-saclay.png"))
            default_logo_idx = 0
        ...
        with st.expander("Directeurs de recherche et encadrement (optionnel pour les devoirs)") :
            ...
            if not isinstance(directors_list, list):
                directors_list = []
            d1_name = directors_list[0].get("name", "") if len(directors_list) > 0 and isinstance(directors_list[0], dict) else (str(directors_list[0]) if len(directors_list) > 0 else "")
            d1_title = directors_list[0].get("title", "") if len(directors_list) > 0 and isinstance(directors_list[0], dict) else ""
            d1_role = directors_list[0].get("role", "Dirigé") if len(directors_list) > 0 and isinstance(directors_list[0], dict) else "Dirigé"

            d2_name = directors_list[1].get("name", "") if len(directors_list) > 1 and isinstance(directors_list[1], dict) else (str(directors_list[1]) if len(directors_list) > 1 else "")
            d2_title = directors_list[1].get("title", "") if len(directors_list) > 1 and isinstance(directors_list[1], dict) else ""
            d2_role = directors_list[1].get("role", "Et co-dirigé") if len(directors_list) > 1 and isinstance(directors_list[1], dict) else "Et co-dirigé"
        ...
        jury_list = thesis_data.get("jury", [])
        if jury_list and isinstance(jury_list, list):
            with st.expander(f"Composition du jury ({len(jury_list)} membre(s) configure(s))"):
                st.caption("Ces membres de jury sont integralement preserves lors de chaque enregistrement et compilation.")
                for m in jury_list:
                    if isinstance(m, dict):
                        st.markdown(f"- **{m.get('name', '')}** -- {m.get('title', '')} ({m.get('role', '')})")
                    else:
                        st.markdown(f"- {m}")
...
def build_updated_yaml():
    ...
    # Auteur : preserver la structure riche si non modifiee
    orig_author = yaml_data.get("author")
    if in_author.strip() == cur_author.strip() and isinstance(orig_author, (list, dict)):
        d["author"] = orig_author
    else:
        d["author"] = in_author.strip()
    ...
    if st.session_state.opt_thesis_active:
        ...
        if sel_logo == "Université Paris-Saclay":
            ...
            if existing_logo and "paris-saclay" in str(existing_logo) and not str(existing_logo).startswith("_") and "_extensions" not in str(existing_logo):
                resolved_logo = existing_logo
            else:
                resolved_logo = "images/logo-universite-paris-saclay.png"
        elif sel_logo == "Université Paris Cité":
            existing_logo = thesis_dict.get("logo", "")
            if existing_logo and "paris-cite" in str(existing_logo) and not str(existing_logo).startswith("_") and "_extensions" not in str(existing_logo):
                resolved_logo = existing_logo
            else:
                resolved_logo = "images/logo-universite-paris-cite.png"
        ...
        if th_laboratory.strip():
            thesis_dict["laboratory"] = th_laboratory.strip()
        ...
        # Directeurs (mise a jour preservee sans perte des directeurs 3+)
        existing_directors = thesis_dict.get("directors", [])
        if not isinstance(existing_directors, list):
            existing_directors = []
        updated_directors = list(existing_directors)

        new_d1 = None
        if th_d1_name.strip():
            new_d1 = {"name": th_d1_name.strip(), "title": th_d1_title.strip(), "role": th_d1_role.strip()}
        new_d2 = None
        if th_d2_name.strip():
            new_d2 = {"name": th_d2_name.strip(), "title": th_d2_title.strip(), "role": th_d2_role.strip()}

        if new_d1:
            if len(updated_directors) > 0:
                updated_directors[0] = new_d1
            else:
                updated_directors.append(new_d1)
        if new_d2:
            if len(updated_directors) > 1:
                updated_directors[1] = new_d2
            else:
                updated_directors.append(new_d2)

        if updated_directors:
            thesis_dict["directors"] = updated_directors
