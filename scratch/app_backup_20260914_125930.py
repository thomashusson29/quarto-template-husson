...
def ensure_symlink(project_dir: Path):
    """Assure que le dossier _extensions/husson est present ou lie dans le dossier du projet."""
    ...
    if (project_dir / "_extensions" / "husson").resolve() == TEMPLATE_EXTENSION_DIR.resolve():
        return True, "Racine du depot template husson."
...
with tab_ollama:
    st.markdown("**Assistant IA local pour Quarto & Template Husson (Ollama) :**")
    ...
    if not ollama_models_tab:
        ...
        st.caption(f"Serveur local actif -- {len(ollama_models_tab)} modele(s) detecte(s).")
        def_idx_tab = 0
        ...
        with col_tol1:
            sel_tab_model = st.selectbox("Modele local :", options=ollama_models_tab, index=def_idx_tab, key="tab_ollama_model")
        with col_tol2:
            st.write("")
            st.write("")
            btn_check_doc = st.button("Analyser la syntaxe YAML", use_container_width=True, key="btn_check_doc_yaml")
        ...
        if btn_check_doc:
            ...
            prompt_check = (
                "Tu es un expert Quarto et XeLaTeX.\n"
                "Analyse cet en-tete YAML d'un document Quarto utilise avec le template Husson :\n"
                "```yaml\n"
                f"{curr_yaml_str}\n"
                "```\n"
                "Identifie d'eventuelles incoherences de syntaxe, options conflictuelles (ex: packages babel, biblio natbib, polices) "
                "et confirme si la configuration est saine. Reponds en francais sans emoji de maniere concise."
            )
            st.markdown(f"**Analyse de syntaxe par `{sel_tab_model}` :**")
            with st.chat_message("assistant"):
                st.write_stream(stream_ollama_response(sel_tab_model, prompt_check))

        free_question = st.text_input("Poser une question sur Quarto, le template ou une commande LaTeX :", key="free_question_input")
        if free_question:
            prompt_q = (
                "Tu es un expert specialise en redaction de documents Quarto, XeLaTeX et mise en page academique.\n"
                f"Question de l'utilisateur : {free_question}\n"
                "Reponds de facon concise, technique et directement applicable en francais. Ne mets aucun emoji."
            )
            with st.chat_message("assistant"):
                st.write_stream(stream_ollama_response(sel_tab_model, prompt_q))
...
def build_updated_yaml():
    ...
    if st.session_state.opt_pdf:
        ...
        # En-tetes LaTeX Husson (pdf-header.tex)
        headers = list(existing_pdf.get("include-in-header", [])) if isinstance(existing_pdf.get("include-in-header"), list) else ([existing_pdf["include-in-header"]] if existing_pdf.get("include-in-header") else [])
        if "_extensions/husson/pdf-header.tex" not in headers:
            headers.append("_extensions/husson/pdf-header.tex")
        ...
        pdf_key = "husson-pdf" if "husson-pdf" in d["format"] else "pdf"
        d["format"][pdf_key] = existing_pdf
    ...
    if st.session_state.opt_html:
        ...
        html_key = "husson-html" if "husson-html" in d["format"] else "html"
        d["format"][html_key] = existing_html
    ...
    if st.session_state.opt_docx:
        ...
        elif "husson-docx" in d["format"] and isinstance(d["format"]["husson-docx"], dict):
        ...
        docx_key = "husson-docx" if "husson-docx" in d["format"] else "docx"
        d["format"][docx_key] = existing_docx
    ...
    if st.session_state.opt_reveal:
        ...
        elif "husson-revealjs" in d["format"] and isinstance(d["format"]["husson-revealjs"], dict):
            existing_reveal = dict(d["format"]["husson-revealjs"])
        existing_reveal["slide-number"] = True
        existing_reveal["transition"] = "none"
        reveal_key = "husson-revealjs" if "husson-revealjs" in d["format"] else "revealjs"
        d["format"][reveal_key] = existing_reveal
...
if btn_render:
    ...
    target_formats = []
    if st.session_state.opt_pdf:
        target_formats.append("husson-pdf" if "husson-pdf" in yaml_data.get("format", {}) else "pdf")
    if st.session_state.opt_html:
        target_formats.append("husson-html" if "husson-html" in yaml_data.get("format", {}) else "html")
    if st.session_state.opt_docx:
        target_formats.append("husson-docx" if "husson-docx" in yaml_data.get("format", {}) else "docx")
    if st.session_state.opt_reveal:
        target_formats.append("husson-revealjs" if "husson-revealjs" in yaml_data.get("format", {}) else "revealjs")
                ...
                with st.chat_message("assistant"):
                    ...
                    def token_collector():
                        for t in stream_ollama_response(diag_model, diag_prompt):
                            full_resp.append(t)
                            yield t
                    st.write_stream(token_collector())
                    st.session_state["last_diagnosis"] = "".join(full_resp)
