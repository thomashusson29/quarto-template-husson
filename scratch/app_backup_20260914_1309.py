...
with tab_ollama:
    ...
    if not ollama_models_tab:
        ...
        with st.form("chat_inline_form", clear_on_submit=True):
            ...
            with col_in1:
                user_text_form = st.text_input(
                    "Votre message :",
                    placeholder="Ecrivez votre message ici... (Appuyez sur Entree ou cliquez sur Envoyer)",
                    label_visibility="collapsed",
                    key="chat_inline_user_text"
                )
            with col_in2:
                btn_send = st.form_submit_button("Envoyer", use_container_width=True)

        if btn_send and user_text_form.strip():
            prompt_to_send = user_text_form.strip()
