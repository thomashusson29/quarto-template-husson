...
function RawBlock(raw)
  ...
  local text = raw.text
  if not text:match("\\begin%{tabular") then
    return nil
  end
  ...
  if #pt_widths > 0 then
    ...
    -- Seuil de débordement : 430 pt (~15.2 cm, largeur maximale utile A4 avec marges)
    if total_pt > 430 then
  ...
  local target_size = (#pt_widths >= 6) and 9 or 10
  local target_skip = (#pt_widths >= 6) and 11 or 12
